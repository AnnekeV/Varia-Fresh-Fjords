...
dsRunoff_RACMO_1k_YY_GrIS = mask_data(dsRunoff['runoffcorr'], "PROMICE_GrIS", spatial_resolution)
dsRunoff_RACMO_1k_YY_GIC = mask_data(dsRunoff['runoffcorr'], "PROMICE_Ice_caps", spatial_resolution)
dsRunoff_RACMO_1k_YY_GrIS_sum = dsRunoff_RACMO_1k_GrIS.sum(dim=['x', 'y']).resample(time='YS').sum()
dsRunoff_RACMO_1k_YY_GIC_sum = dsRunoff_RACMO_1k_GIC.sum(dim=['x', 'y']).resample(time='YS').sum()

dsRunoff_RACMO_1k_YY_GrIS_sections = dsRunoff_RACMO_1k_GrIS.groupby(dsmask_sections['section_numbers_adjusted']).sum()
dsRunoff_RACMO_1k_YY_GIC_sections = dsRunoff_RACMO_1k_GIC.groupby(dsmask_sections['section_numbers_adjusted']).sum()


# %%
folder500m = pathIMAU02 +"RACMO2.3p2/FGRN055/Downscaling_GR_500m/"
openMAR = True
dsRunoff500mRACMO = xr.open_mfdataset(folder500m + "Annual/runoff.1940-2023.BN_RACMO2.3p2_ERA5_3h_1940_FGRN055.GrIS.0.5km.YY_copy.nc")
...
# %% PRECIPITATION CARRA
ds_precip_carra_1991_2008_sum = xr.open_dataset(pathIMAU02 +"CARRA/Yearly/RACMOgrid/fjords_only/total_precipitation.CARRA.west_domain.1991-2008.1km.YY.fjords_only.sum_per_basin.nc")
ds_precip_carra_2009_2023_sum = xr.open_dataset(pathIMAU02 +"CARRA/Yearly/RACMOgrid/fjords_only/total_precipitation.CARRA.west_domain.2009-2023.1km.YY.fjords_only.sum_per_basin.nc")
# concatenate
ds_precip_carra_1991_2023_sum = xr.concat([ds_precip_carra_1991_2008_sum, ds_precip_carra_2009_2023_sum], dim='time')
...
#  GrIS
dsRunoffIceSectormm = xr.open_dataset(pathIMAU02 +"RACMO2.3p2/FGRN055/Downscaling_GR/Monthly/runoff_GrIS.1990-2023.RACMO2.3p2_ERA5_3h_FGRN055.1km.MM.GrIS.section_sum.nc")['runoffcorr']
...
# Tundra
fname = pathIMAU02 +"RACMO2.3p2/FGRN055/Downscaling_GR/Mask_adjusted_section_numbers_slater_may24_copy.nc"
...
ds_precip_carra_1991_2023_sum = xr.open_mfdataset([pathIMAU02 +"CARRA/Yearly/RACMOgrid/fjords_only/total_precipitation.CARRA.west_domain.1991-2008.1km.YY.fjords_only.sum_per_basin.nc", pathIMAU02 +"CARRA/Yearly/RACMOgrid/fjords_only/total_precipitation.CARRA.west_domain.2009-2023.1km.YY.fjords_only.sum_per_basin.nc"])/1e6
...
dsPrecipFjordsSectormm_sum = dsPrecipFjordsSectormm_sum.resample(time='YS').sum()
# %%
monthly_precip_cara1 = pathIMAU02 +"CARRA/Monthly/RACMOgrid/fjords_only/total_precipitation.CARRA.west_domain.2009-2023.1km.MM.fjords_only.sum_per_basin.nc"
ds_precip_carra_2009_2023_month = xr.open_dataset(monthly_precip_cara1)/1e6

ds_precip_carra_1991_2008_month = xr.open_dataset(pathIMAU02 +"CARRA/Monthly/RACMOgrid/fjords_only/total_precipitation.CARRA.west_domain.1991-2008.1km.MM.fjords_only.sum_per_basin.nc")/1e6

ds_precip_carra_1991_2023_month = xr.concat([ds_precip_carra_1991_2008_month, ds_precip_carra_2009_2023_month], dim='time')

ds_precip_racmo_1990_2023_sum_monthly = xr.open_mfdataset(pathIMAU02 +"RACMO2.3p2/FGRN055/Downscaling_GR/Monthly/fjords_only/precip.1990-2023.BN_RACMO2.3p2_ERA5_3h_FGRN055.1km.MM.fjords_only.sum_per_basin.nc", decode_times=False)
ds_precip_racmo_1990_2023_sum_monthly['time'] = pd.to_datetime('1990-01-01') + pd.to_timedelta(ds_precip_racmo_1990_2023_sum_monthly.time, 'D')
...
dfPrecipFjordsSector_RACMO_month = ds_precip_racmo_1990_2023_sum_monthly.to_dataframe().reset_index().set_index(['time', 'section_numbers_adjusted'])[['precipcorr']].reset_index().set_index('time').drop_duplicates().pivot(columns='section_numbers_adjusted').rename(columns=dict_sections)
...
data_mean_seasonal_period2 = dsMonthlyGr.sel(time=slice(period2['start'],period2['end'])).sum(dim='Basins').groupby('time.month').mean()
data_std_seasonal_period2 = dsMonthlyGr.sel(time=slice(period2['start'],period2['end'])).sum(dim='Basins').groupby('time.month').std()
# do the same for 2002-2012
data_mean_seasonal_period1 = dsMonthlyGr.sel(time=slice(period1['start'], period1['end'])).sum(dim='Basins').groupby('time.month').mean()
data_std_seasonal_period2 = dsMonthlyGr.sel(time=slice(period1['start'], period1['end'])).sum(dim='Basins').groupby('time.month').std()

dfGIS_period1 =  pd.read_csv(path_Mankoff2020Solid + "GIS_D.csv", index_col=0, parse_dates=True)[period1['start']:period1['end']]
dfGIS_period2 =  pd.read_csv(path_Mankoff2020Solid + "GIS_D.csv", index_col=0, parse_dates=True)[period2['start']:period2['end']]
data_mean_seasonal_period1['Solid Ice Discharge'].values = np.squeeze(dfGIS_period1.groupby(dfGIS_period1.index.month).mean().values) /12
data_mean_seasonal_period2['Solid Ice Discharge'].values =np.squeeze(dfGIS_period2.groupby(dfGIS_period2.index.month).mean().values) /12
data_std_seasonal_period2['Solid Ice Discharge'].values = np.squeeze(dfGIS_period2.groupby(dfGIS_period2.index.month).std().values) /12
data_std_seasonal_period2['Solid Ice Discharge'].values =np.squeeze(dfGIS_period2.groupby(dfGIS_period2.index.month).std().values) /12
# %%
